...
contract Dao is IDao, SemanticSBTUpgradeable {
    ...
    string  constant GROUP_PREFIX = "relationlabs-dao-";
    ...
    function createGroupOnGNFD(uint256 groupIndex) external payable {
        require(groupName.strlen() == 0,"Dao:already created group on gnfd");
    ...
    function setDaoURI(string calldata daoURI_) external {
        require(msg.sender == ownerOfDao || _minters[msg.sender], "Dao: permission denied");
        _setDaoURIInternal(daoURI_);
    }
    ...
    function _setDaoURIInternal(string memory daoURI_) internal {
        daoURI = daoURI_;
