...
library SemanticSBTLogicUpgradeable {
    ...
    function buildIntRDF(uint256 oIndex, string storage name) internal pure returns (string memory){
        string memory p = string.concat(PROPERTY_PREFIX, name);
        string memory o = oIndex.toString();
        return string.concat(p, BLANK_SPACE, o);
    }

    function buildStringRDF(string storage name, string storage _stringO) internal pure returns (string memory){
        string memory p = string.concat(PROPERTY_PREFIX, name);
