...
contract SemanticSBTUpgradeable is Initializable, OwnableUpgradeable, ERC165Upgradeable,
ERC721Upgradeable, IERC721EnumerableUpgradeable, ISemanticSBT, ISemanticSBTSchema, IERC5192,GroupApp {
    ...
    // reference: https://greenfield.bnbchain.org/docs/guide/dapp/contract-list.html
    address constant public GROUP_HUB = 0x014964f4596A1fE218867867696b0661cF2421CA;
    address constant public CROSS_CHAIN_HUB = 0x24e4b644DF338f9656843E2Ebf1b84715B8c58Ba;
    address constant public ERC721_TOKEN = 0xC9654fe0C1450D387158c9D7492B3Df77CF0B030;
    uint256 constant public CALL_BACK_GAS_LIMIT = 500000;
    uint8 constant public FAILURE_HANDLE_STRATEGY = 2;
    ...
    function initialize(
        ...
        __base_app_init_unchained(CROSS_CHAIN_HUB, CALL_BACK_GAS_LIMIT, FAILURE_HANDLE_STRATEGY);
    ...
    function locked(uint256 tokenId) external override view returns (bool){
        require(_exists(tokenId), "SemanticSBT: does not exist");
        if (_transferable) {
            return true;
        }
        return false;
    ...
    function _createGroupCallback(uint32 _status, uint256 _tokenId, bytes memory _callbackData) internal override virtual {
        groupId = _tokenId;
    }


    function _updateGroupMember(address addr, GroupStorage.UpdateGroupOpType operate) internal {
        uint256 tokenId = IERC721EnumerableUpgradeable(ERC721_TOKEN).tokenOfOwnerByIndex(address(this),0);
        require( tokenId > 0,"SemanticSBT: group not created on gnfd ");
        if (groupId == 0){
            groupId = tokenId;
        }
        address[] memory members = new address[](1);
        members[0] = addr;
        super._updateGroup(address(this), groupId, operate, members);
    }
